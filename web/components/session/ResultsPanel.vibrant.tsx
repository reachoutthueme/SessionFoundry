...
export default function ResultsPanel({ sessionId }: { sessionId: string }) {
  ...
  async function toggle(id: string) {
    ...
    if (next && !data[id]) {
      ...
      setData((d) => ({
        ...d,
        [id]: j.stocktake ?? j.submissions ?? [],
      }));
      setLoading(false);
    }
  }

  // Poll open activities (live update during voting)
  useEffect(() => {
                  ...
                  <div className="p-3 border-t border-white/10">
                    {(() => {
                      ...
                      const stVals = rows
                        .map((s: Sub) => s.stdev ?? 0)
                        .filter((v: number) => isFinite(v));
                      const nVals = rows.map((s: Sub) => s.n ?? 0);
                      const consVals = stVals.map(
                        (v: number) => 1 / (1 + v)
                      );
                      ...
                      const badge = (
                        ...
                        if (score >= 0.67)
                          ...
                          tone =
                            "border-red-400/30 bg-red-500/15 text-[var(--text)]";
                              ...
                              >
                                ...
                                <div className="mt-1 text-xs text-[var(--muted)] flex items-center gap-2">
                                  ...
                                  {badge(
                                    ...
                                    s.stdev === null
                                      ? "-"
                                      : (s.stdev as number).toFixed(2),
                                    norm(s.stdev ?? 0, stMM, true)
                                  )}
                                  {badge(
                                    "N",
                                    String(s.n ?? 0),
...
function Scatter({
  ...
  // RowPoint is the shape after we decorate each point with idx + consensus
  type RowPoint = {
    idx: number;
    id: string;
    label: string;
  ...
  const rows: RowPoint[] = useMemo(
    () =>
      points.map((p, i): RowPoint => ({
        idx: i + 1,
        ...p,
    ...
    <Card>
      ...
      <CardBody className="overflow-x-auto">
        ...
        >
          ...
          {/* Y ticks (avg) */}
          {Array.from({ length: ticksY + 1 }).map((_, i: number) => {
          ...
          {/* X ticks (consensus) */}
          {Array.from({ length: ticksX + 1 }).map((_, i: number) => {
