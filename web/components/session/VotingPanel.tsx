...
export default function VotingPanel({ sessionId, activityId }: { sessionId: string; activityId?: string }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setSubs(list);
      // default all to 0 for allocation
  ...
  return (
    <Card>
      <CardHeader title="Voting" subtitle={budget>0?`Distribute ${budget} points across items`:'Rate each item (0â€“10)'} />
      <CardBody className="space-y-4">
        {loading ? (
          ...
          <>
            {budget > 0 && (
              <div className={`p-2 rounded-md text-xs mb-2 ${remaining===0?"bg-green-500/10 text-green-200 border border-green-400/30":"bg-white/5 text-[var(--muted)] border border-white/10"}`}>
            ...
            {showLeaderboard && leaderboard && leaderboard.length > 0 && (
              <div className="mt-6">
                <Card>
                  <CardHeader title="Leaderboard" subtitle="Ranked by total points" />
